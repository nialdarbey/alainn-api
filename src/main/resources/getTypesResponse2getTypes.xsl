...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="pageSize" select="/mes:getTypesResponse/mes:PageInfo/pageSize" />
	...
	<xsl:template match="/">
		...
		<xsl:if test="$pageIndex > 0" >
			...
			"collection" : {
				"size" : <xsl:value-of select="/mes:getTypesResponse/mes:PageInfo/pageSize" />,
				"items" : [
		<xsl:for-each select="mes:getTypesResponse/mod:Type">
			"<xsl:value-of select="." />"<xsl:if test="./following-sibling::mod:Type">,</xsl:if>
		</xsl:for-each>
				]
			}
		}
